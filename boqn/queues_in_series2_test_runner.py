...
for i in range(2):
    for k in range(n_queues):
        active_input_indices.append([j for j in range(k + 1)] + [n_queues + j for j in range(k + 1)])
...
def output_for_EIQN(simulator_output):
    output = torch.empty(simulator_output.shape[:-2] + torch.Size([2 * n_queues]))
    output[..., :n_queues] =  simulator_output[..., 0]
    output[..., n_queues:] =  simulator_output[..., 2]
    return output
