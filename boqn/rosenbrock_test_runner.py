...
test_problem = 'rosenbrock_' + str(input_dim)
...
main_input_indices = []
for k in range(n_nodes):
    main_input_indices.append([k, k + 1]) 
...
def update_random_observations(best_Random):
    ...
    simulator_output = rosenbrock.evaluate(x)
...
if len(sys.argv) > 1:
    for trial in range(first_trial, last_trial + 1):
        ...
        simulator_output_at_X = rosenbrock.evaluate(X)
        
        if run_EIQN:
            try:
                best_observed_EIQN = list(np.loadtxt(results_folder + test_problem + '_EIQN_' + str(trial) + '.txt'))
                running_times_EIQN = list(np.loadtxt(results_folder + 'running_times/' + test_problem + '_rt_EIQN_' + str(trial) + '.txt'))
                best_value_EIQN = torch.tensor(best_observed_EIQN[-1])
                X_EIQN = torch.tensor(np.loadtxt(results_folder + 'X/' + test_problem + '_X_EIQN_' + str(trial) + '.txt'))
                fX_EIQN = torch.tensor(np.loadtxt(results_folder + 'Y/' + test_problem + '_Y_EIQN_' + str(trial) + '.txt'))
                init_batch_id = len(best_observed_EIQN)
            except:
                best_observed_EIQN = []
                running_times_EIQN = []
                X_EIQN = X.clone()
        ...
        for iteration in range(init_batch_id, N_BATCH + 1):
            ...
            if run_EIQN:
                ...
                new_fx_EIQN = rosenbrock.evaluate(new_x_EIQN)
            ...
            if run_EI:
                ...
                new_fx_EI = output_for_EI(rosenbrock.evaluate(new_x_EI))
            ...
            if run_KG:
                ...
                new_fx_KG = output_for_EI(rosenbrock.evaluate(new_x_KG))
