    ...
    def __init__(self, train_X, train_Y, dag, active_input_indices, train_Yvar=None, node_GPs=None, normalization_constant_lower=None, normalization_constant_upper=None) -> None:
        ...
        if node_GPs is not None:
            ...
            for k in self.root_nodes:
                ...
                self.node_GPs[k] = FixedNoiseGP(train_X=train_X_node_k, train_Y=train_Y_node_k, train_Yvar=torch.ones(train_Y_node_k.shape) * 1e-5, outcome_transform=Standardize(m=1))
            ...
            for k in range(self.n_nodes):
                if self.node_GPs[k] is None:
                    ...
                    train_Y_node_k = train_Y[..., [k]]
                    aux_model =  FixedNoiseGP(train_X=train_X_node_k, train_Y=train_Y_node_k, train_Yvar=torch.ones(train_Y_node_k.shape) * 1e-5, outcome_transform=Standardize(m=1))  
                    ...
                    self.node_GPs[k] = FixedNoiseGP(train_X=train_X_node_k, train_Y=train_Y_node_k, train_Yvar=torch.ones(train_Y_node_k.shape) * 1e-5, outcome_transform=Standardize(m=1, batch_shape=torch.Size([])))
                    self.node_mlls[k] = ExactMarginalLogLikelihood(self.node_GPs[k].likelihood, self.node_GPs[k])
                    fit_gpytorch_model(self.node_mlls[k])
    ...
    def condition_on_observations(self, X: Tensor, Y: Tensor, **kwargs: Any) -> Model:
        ...
        for k in self.root_nodes:
            if self.active_input_indices is not None:
                ...
                X_node_k = X
            Y_node_k = Y[..., [k]]
            fantasy_models[k] = self.node_GPs[k].condition_on_observations(X_node_k, Y_node_k, noise=torch.ones(Y_node_k.shape[1:]) * 1e-5)
        ...
        return NetworkGP(dag=self.dag, train_X=X, train_Y=Y, active_input_indices=self.active_input_indices, node_GPs=fantasy_models, normalization_constant_lower=self.normalization_constant_lower, normalization_constant_upper=self.normalization_constant_upper)
