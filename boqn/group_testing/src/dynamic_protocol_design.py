...
def simple_simulation(group_sizes=[80,90,100,110], prevalence=0.01, resolved=0., seed=1, verbose=False):
    ...
    for t in range(T):
        state[t] =  list(state[t].values())
    return state[0:T], loss


def node(state, group_size, costs = {'test' : .001, 'infections' : 1, 'quarantine' : .01}, verbose=False):
    # Takes as input:
    ...
    people_in_quarantine  = new_state['QI'] + new_state['QS'] + new_state['QR']
